# Guía de ejercicios

[Algoritmos III Corsi-Essaya](https://algo3.super.site/)

## 1. Objetos simples

1. [FIUBA](src/main/java/ObjetosSimples/FIUBA#readme)
1. [Chat](src/main/java/ObjetosSimples/Chat#readme)
1. [Cartelera](src/main/java/ObjetosSimples/Cartelera#readme)
1. [Biblioteca](src/main/java/ObjetosSimples/Biblioteca#readme)
1. [Caja de ahorro](src/main/java/ObjetosSimples/Cuenta#readme)
1. [Fecha](src/main/java/ObjetosSimples/Fecha#readme)

## 2. Herencia y polimorfismo

1. [Figuras](src/main/java/HerenciaYPolimorfismo/AreaTotal#readme)
1. [ListaDuplicante](src/main/java/HerenciaYPolimorfismo/ListaDuplicante#readme)
1. [FIUBA 2](src/main/java/HerenciaYPolimorfismo/FIUBA2#readme)
1. [Cartelera 2](src/main/java/HerenciaYPolimorfismo/Cartelera2#readme)
1. ☣️ [Ajedrez](src/main/java/HerenciaYPolimorfismo/Ajedrez#readme)

## 3. TDD

1. [ROT13](src/main/java/TDD/ROT13#readme)
1. [Notación Romana](src/main/java/TDD/NotacionRomana#readme)
1. [Tenis](src/main/java/TDD/PuntajeTenis#readme)
1. [Bowling](src/main/java/TDD/Bowling#readme)
1. [Calculadora](src/main/java/TDD/Calculadora#readme)
1. [Contraseña](src/main/java/TDD/Contrasenia#readme)

## 4. Principios de diseño

1. [Auto](src/main/java/PrincipiosDeDiseño/Auto#readme)
1. [Carrito de compras](src/main/java/PrincipiosDeDiseño/Carrito#readme)
1. [Cubo](src/main/java/PrincipiosDeDiseño/Cubo#readme)
1. [Cuenta bancaria](src/main/java/PrincipiosDeDiseño/CuentaBancaria#readme)
1. [Empleado](src/main/java/PrincipiosDeDiseño/Empleado#readme)
1. [Producto](src/main/java/PrincipiosDeDiseño/Producto#readme)
1. [Rectangulo y cuadrado](src/main/java/PrincipiosDeDiseño/RectanguloYCuadrado#readme)
1. [Suma](src/main/java/PrincipiosDeDiseño/Suma#readme)
1. [Usuario](src/main/java/PrincipiosDeDiseño/Usuario#readme)
1. [Usuario 2](src/main/java/PrincipiosDeDiseño/Usuario2#readme)
1. [Vehiculo](src/main/java/PrincipiosDeDiseño/Vehiculo#readme)

## 5. Patrones de diseño
1. [Futbol](src/main/java/PatronesDeDiseño/Futbol#readme)
2. [Cafe](src/main/java/PatronesDeDiseño/Cafe#readme)
3. [Sensores](src/main/java/PatronesDeDiseño/Sensores#readme)
4. [Figuras](src/main/java/PatronesDeDiseño/Figuras#readme)
5. [Impresoras](src/main/java/PatronesDeDiseño/Impresoras#readme)

## 6. Excepciones
1. [Salida](src/main/java/Excepciones/Salida#readme)
1. [Throws](src/main/java/Excepciones/Throws#readme)

## 7. Persistencia
1. [Persistencia](src/main/java/Persistencia#readme)

## 8. Interfaces gráficas
1. [Interfaces gráficas](src/main/java/InterfacesGraficas#readme)

## 9. Programación funcional
1. [Refactorizar](src/main/java/Funcional/Refactorizar#readme)
1. [Impuras](src/main/java/Funcional/Impuras#readme)
